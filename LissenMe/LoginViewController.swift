...
class LoginViewController: UIViewController, MaskedTextFieldDelegateListener {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        {
            performSegue(withIdentifier: "loginGoQr", sender: self)
        }
        else
        {
    ...
    @IBAction func register_action(_ sender: Any) {
        ...
        if self.customer_phone.text! == ""  || self.customer_pass.text! == ""
        {
        ...
        {
            goUrl = "http://lissen.me/Main/registerUser?phone="+self.customer_phone.text!+"&pass="+self.customer_pass.text!+""
            ...
            Alamofire.request(goUrl).responseJSON { response in
                ...
                if let data = response.data, let utf8Text = String(data: data, encoding: .utf8) {
                    var myrStringArr = utf8Text.components(separatedBy: ":")
                    if myrStringArr[0] == "1"
                    {
                        print(myrStringArr[1])
                        UserDefaults.standard.set(myrStringArr[1], forKey: "customer_name")
                        UserDefaults.standard.set(self.customer_phone.text, forKey: "customer_phone")
                        
                        self.performSegue(withIdentifier: "loginGoQr", sender: self)
                    }
                    else
                    {
                        let alert2 = UIAlertController(title: "Dikkat", message: "Girdiğiniz bilgiler hatalı, lütfen tekrar deneyin.", preferredStyle: .alert)
    ...
    @IBAction func loginToRegister(_ sender: Any) {
